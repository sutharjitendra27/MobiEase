...
const Breadcrumbs = () => {
  ...
  useEffect(() => {
    const currentMenuItem: any = menuItems.find((item) => item.link === locationName)

    const currentChild: any = menuItems.find((item: any) =>
      item.child?.find((child: { childlink: string }) => child.childlink === locationName)
    )
